...
namespace NLog.Targets.ElasticSearch
{
    ...
    public class ElasticSearchTarget : TargetWithLayout, IElasticSearchTarget
    {
        ...
        private IDictionary<string, object> AsDictionary(object source, BindingFlags bindingAttr = BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.Instance)
        {
            return source.GetType().GetProperties(bindingAttr).ToDictionary
            (
                propInfo => propInfo.Name,
                propInfo => (object)JsonConvert.SerializeObject(propInfo.GetValue(source, null))
            );
        }
        ...
        private object FormPayload(IEnumerable<LogEventInfo> logEvents)
        {
            ...
            {
                var document = AsDictionary(logEvent);
